...
(function() {
    ...
    (function() {
        ...
        panHelperBtnArr = $.merge($('.icon-download').parent('a'), $('.icon-btn-download').parent('li'));
        o.id = 'panHelperMenu';
    ...
    function helperDownload(type) {
        ...
        if(isOneFile) {
            ...
            if(isOther) {
                ...
                action = '',
                data = [],
                packName = getDownloadName(items);
                for (var i = 0; i < len; i++) {
                    data.push({
